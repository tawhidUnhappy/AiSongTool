/**
 * Persistent "generated songs" library — every song this app produces (via
 * ACE-Step's sample-mode API) or that the user picked lands in
 * `<outputDir>/audio/` so it can be browsed and reused as a fresh input
 * later. `<outputDir>/images/` and `<outputDir>/videos/` are the matching
